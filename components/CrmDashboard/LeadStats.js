...
export default function LeadStats({ stats }) {
  ...
  const gradeDistribution = [
    ...
    { name: 'Warm', value: stats.warmLeads, color: '#F97316' },
  ...
  const conversionFunnel = [
    ...
    { stage: 'Contacted', count: stats.contactedLeads || 0, percentage: totalLeads > 0 ? Math.round(((stats.contactedLeads || 0) / totalLeads) * 100) : 0 },
    { stage: 'Qualified', count: stats.qualifiedLeads || 0, percentage: totalLeads > 0 ? Math.round(((stats.qualifiedLeads || 0) / totalLeads) * 100) : 0 },
    { stage: 'Converted', count: stats.convertedLeads || 0, percentage: totalLeads > 0 ? Math.round(((stats.convertedLeads || 0) / totalLeads) * 100) : 0 }
  ];
              ...
              <div className="w-full bg-gray-200 rounded-full h-8 relative">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: index === 0 ? '#3B82F6' : index === 1 ? '#8B5CF6' : index === 2 ? '#10B981' : '#059669',
                ...
                {stage.percentage === 0 && (
                  <span className="absolute left-2 top-1/2 -translate-y-1/2 text-sm text-gray-500">0%</span>
                )}
              </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <InsightCard
            ...
            value={totalLeads > 0 ? `${((stats.convertedLeads || 0) / totalLeads * 100).toFixed(1)}%` : '0%'}
          ...
          <InsightCard
            ...
            value={sourceDistribution.length > 0 ? sourceDistribution.reduce((max, item) => item.value > max.value ? item : max, sourceDistribution[0]).name : 'N/A'}
            description="Best performing channel"
          />
          <InsightCard
            title="Week-over-Week"
            value={`${(stats.weekOverWeekGrowth || 0) > 0 ? '+' : ''}${stats.weekOverWeekGrowth || 0}%`}
            description="Lead growth rate"
          />
          <InsightCard
            title="Contact Rate"
            value={totalLeads > 0 ? `${((stats.contactedLeads || 0) / totalLeads * 100).toFixed(1)}%` : '0%'}
