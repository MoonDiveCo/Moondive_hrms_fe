...
export default function LeadDetail({ leadId, leadData, onClose, onUpdate }) {
  ...
  const handleAddNote = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_MOONDIVE_API}/leads/${leadId}/note`,
        {
          method: "POST",
  ...
  const handleSetFollowUp = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_MOONDIVE_API}/leads/${leadId}/follow-up`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-primary text-white p-6 flex justify-between items-start">
          <div className="flex-1">
            ...
            <p className="text-blue-100">
              {lead.jobTitle || "Unknown Position"}{" "}
              {lead.companyName ? `at ${lead.companyName}` : ""}
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-white cursor-pointer rounded-lg p-2 transition-colors"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="border-b border-gray-200 p-4 flex gap-2 flex-wrap">
          ...
          <select
            ...
            className="px-4 py-2 rounded-lg text-primaryText border border-primary  bg-whiteBg focus:outline-none">
            ...
            {[
              ...
              <button
                ...
                className={`
                  ...
                  ${
                    activeTab === tab.id
                     ? "order-primary text-primary"
    : "border-transparent text-primaryText hover:text-blackText "
                  }
                `}
              >
                {tab.label}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <InfoItem
                    label="Last Activity"
                    value={formatDate(lead.lastActivityAt || lead.createdAt)}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <MetricCard
                    label="Pages Viewed"
                    value={lead.pagesVisited?.length || 0}
                  />
                  <MetricCard
                    label="Total Time on Site"
                    value={`${lead.totalTimeOnSite || 0}s`}
                  />
                  <MetricCard
                    label="Downloads"
                    value={lead.downloadedResources?.length || 0}
                  />
                  <MetricCard
                    label="Chat Conversations"
                    value={lead.chatbotConversations?.length || 0}
                  />
                  <MetricCard
                    label="Pricing Viewed"
                    value={lead.viewedPricing ? "Yes" : "No"}
                  />
                  <MetricCard
                    label="Demo Requested"
                    value={lead.requestedDemo ? "Yes" : "No"}
                  />
                </div>
              </div>
              ...
              {(lead.utmSource || lead.utmMedium || lead.utmCampaign) && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {lead.utmSource && (
                      <InfoItem label="UTM Source" value={lead.utmSource} />
                    )}
                    {lead.utmMedium && (
                      <InfoItem label="UTM Medium" value={lead.utmMedium} />
                    )}
                    {lead.utmCampaign && (
                      <InfoItem label="UTM Campaign" value={lead.utmCampaign} />
                    )}
                    {lead.utmTerm && (
                      <InfoItem label="UTM Term" value={lead.utmTerm} />
                    )}
                  </div>
                </div>
              )}
          ...
          {activeTab === "chat" && (
            <div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                {lead.behavior?.interactions &&
                Array.isArray(lead.behavior.interactions) &&
                lead.behavior.interactions.length > 0 ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                      {lead.message
                        ? `Last message: "${lead.message.substring(0, 100)}${
              ...
              {!lead.emailHistory || lead.emailHistory.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Email history will appear here once you contact this lead
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              Sent {formatDate(email.sentAt)} by{" "}
                              {email.sentBy || "Admin"}
                            </p>
                          </div>
                        </div>
            ...
            <div>
              ...
              <form onSubmit={handleAddNote} className="mb-6">
                ...
                <button
                  ...
                  className="mt-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
