...
export default function EmploymentGroupModal({
  mode,
  data,
  policy,
  onClose,
  ...
  const updateQuota = (code, field, value) => {
    setAllocations((prev) =>
      prev.map((a) => (a.leaveTypeCode === code ? { ...a, [field]: value } : a))
    );
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-[850px] p-6 relative">
              ...
              <div key={lt.code} className="bg-gray-50 p-3 rounded-md">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <input
                    ...
                    disabled={isView}
                    onChange={(e) => toggleUnlimited(lt.code, e.target.checked)}
                    className="h-4 w-4"
                  />
                ...
                <div className="grid grid-cols-2 gap-4 mt-3">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-md mt-1  ${getDisabledClasses(
                        isView || isUnlimited || isOptionalLeave
                      )}`}
                    />
                  </div>
        ...
        <div className="flex justify-end gap-2 mt-5">
          {!isView && (
            <button
              onClick={save}
              className="px-3 text-xs py-2 bg-primary text-white rounded-full"
            >
              Save
            </button>
          )}
          ...
          <button
            onClick={onClose}
            className="px-3 py-2 text-xs text-primaryText bg-white border border-primary rounded-full"
          >
            Close
          </button>
        </div>
