...
export default function OrganizationPolicy() {
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      console.log("File to be uploadedxxxxxxxxxxx:", file);
    ...
    headers: {
     ...
     await sendPolicyNotification(form.fileName);
      setOpenModal(false);
      setForm({
        fileName: '',
        description: '',
 ...
 const sendPolicyNotification = async (policyName) => {
  try {
    

    const payload={
      receiverId: "ALL", // 🔑 backend will fan-out to all users
      notificationTitle: "New Policy Added",
      notificationMessage: `${policyName} has been added to organization policies.`,
      relatedDomainType: "policy", // 🔑
      priority: "Low",
      policyId: null, // or pass policyId if you have it
    }

     await storeNotification(payload);

    console.log("✅ Policy notification triggered");
  } catch (err) {
